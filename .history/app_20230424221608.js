...
`;
    ...
    serviceImgCover.addEventListener("mousemove", function (e) {
        ...
        const pY = e.screenY;
        console.log(pY);
        const ratioX = serviceImg.offsetWidth / serviceImgCover.offsetWidth;
        const ratioY = serviceImg.offsetHeight / serviceImgCover.offsetHeight;
        console.log(ratioX);
        const spaceX =
            (serviceImg.offsetWidth - serviceImgCover.offsetWidth) / 2;
        const spaceY =
            (serviceImg.offsetHeight - serviceImgCover.offsetHeight) / 2;
        let x = (pX / 2) * ratioX - spaceX;
        let y = (pY / 2) * ratioY - spaceY;
        serviceImg.style = `left : ${-x}px;top:${-y}px;max-height:unset;max-width:unset;width:auto;height:auto;transform:unset`;
    });
});
