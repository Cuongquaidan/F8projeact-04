...
pricingSwitch.addEventListener("click", function () {
    let cnt = 0;
    const PriceOldOne = [...pricingPrice][0];
    const PriceOldTow = [...pricingPrice][1];
    const PriceOldThree = [...pricingPrice][2];
    if (cnt < 1) {
        [...pricingPrice].forEach((item) => {
            let price = Math.ceil(parseInt(item.innerText) / 3);
